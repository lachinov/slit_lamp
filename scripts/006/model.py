...
data_transform = Compose([
    ...
    RandCropByPosNegLabeld(keys=["image", "mask"], label_key='mask', pos=0.1, neg=0.9, spatial_size=[192, 192],
                           num_samples=4),
...
class Model(nn.Module):
    def __init__(self, arch, encoder_name, in_channels, out_classes, **kwargs):
        ...
        self.model = smp.create_model(
            arch, encoder_name=encoder_name, in_channels=in_channels, classes=kwargs['decoder_channels'][-1], **kwargs
        )

        self.conv = nn.Sequential(*[
            monai.networks.blocks.Convolution(spatial_dims=2,adn_ordering='NDA',dropout=0.1, in_channels=kwargs['decoder_channels'][-1],
                                              out_channels=kwargs['decoder_channels'][-1],norm='BATCH',kernel_size=3) for i in range(2)
        ])

        self.clf = nn.Conv2d(in_channels=kwargs['decoder_channels'][-1],out_channels=out_classes,kernel_size=1,bias=True)
